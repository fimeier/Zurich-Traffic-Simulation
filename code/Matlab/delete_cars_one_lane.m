function [New_Map, Warteschlange, Output_Count]  = delete_cars_one_lane(modus, New_Map, i, Warteschlange, Output_Count, free_road, x, steps )
...
for street = abc
    if ( Warteschlange ~= 0 && New_Map(street,end) ~= free_road )
      ...
      if ( Warteschlange ~= 0 && New_Map(street,end-1) ~= free_road )
        New_Map(street,end-1) = free_road;
        Warteschlange = Warteschlange - 1;
        Output_Count = Output_Count + 1;        
    end
