%function [West_direction, East_direction] = Cars_in_the_simulation(x_w, output_w_h, x_o, output_o_h, modus)
...
%Richtung Osten alt
[traffic_light_w, M_w, map_size_w] = simulation_sihlstrasse( v_max, free_road, w_modus );
trafficlight_t_w=15;
redlight_t_w=4;
...
%% east direction
    ...
    %Debugin Plot: jeden Step grafisch anzeigen
      ...
      M_plot = New_Map;
      hold on
      %subplot(10,1,mod(i,9)+1)
      if (traffic_light_o(2,1) == 0 )
          title(['\fontsize{16}Traffic Light Position: {\color{red}1, 29, 59}'])
      else
          title(['\fontsize{16}Traffic Light Position: {\color{green}1, 29, 59}'])
      end
      visualization_test( M_plot );
      hold off
...
subplot(2,1,1)
rgb_Matrix_w = visualization_test( M_w );
